...
<div class="container mt-4">
  <div class="card shadow-sm">
    <div class="card-header d-flex justify-content-between align-items-center">
      <h3 class="mb-0"><i class="bi bi-diagram-2 me-2"></i>Strategic SubPrograms</h3>
    ...
    <div class="card-body">
      ...
      <div class="alert alert-info">SubProgram source values are read from the configured segment mapping. Parent Program and description are maintained here as Strategy records.</div>
      <div class="alert alert-warning py-2">Bulk import uses <code>ParentSegmentNo</code> and <code>ParentSegmentCode</code> from <code>tblSegmentValues</code>. Any row whose parent program record has not been imported yet will be skipped and counted in the import result.</div>
      <form method="get" action="index.php" class="row g-2 mb-3">
        <input type="hidden" name="route" value="strategy-setup/sub-programs">
        <div class="col-md-5"><input type="text" name="q" value="<?= h((string)($q ?? '')) ?>" class="form-control" placeholder="Search subprograms"></div>
        <div class="col-md-5">
          <select name="program_id" class="form-select">
            <option value="">All configured programs</option>
      ...
      <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        <div class="small text-muted">Bulk import creates missing subprogram records where the parent program can be resolved from the source parent fields.</div>
        <form method="post" action="index.php?route=strategy-setup/import-sub-program-overlays" class="js-strategy-import-form">
          <input type="hidden" name="_csrf" value="<?= $csrf ?>">
          <input type="hidden" name="reset_mode" value="none" class="js-strategy-reset-mode">
          <button type="button" class="btn btn-sm btn-outline-success js-strategy-import-trigger" data-confirm-title="Import SubPrograms" data-confirm-message="Create missing subprogram records where the parent program can be resolved from the source parent fields?"><i class="bi bi-download me-1"></i>Import Dimensions</button>
        </form>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-striped table-hover align-middle">
          <thead class="table-light"><tr><th>SubProgram</th><th>Org Unit</th><th>Program Overlay</th><th>Overlay</th><th class="text-end">Actions</th></tr></thead>
          <tbody>
          <?php if (empty($records)): ?>
            <tr><td colspan="5" class="text-center text-muted py-3">No subprograms found.</td></tr>
          <?php else: foreach ($records as $row): ?>
            <tr>
              <td><div class="fw-semibold"><?= h((string)($row['SubProgramName'] ?? '')) ?></div><?php if (!empty($row['SubProgramCode'])): ?><div class="small text-muted"><?= h((string)$row['SubProgramCode']) ?></div><?php endif; ?></td>
              <td><div><?= h((string)($row['OrgUnitName'] ?? '')) ?></div><div class="small text-muted"><?= h((string)($row['OrgUnitDataObjectCode'] ?? '')) ?></div></td>
              <td><?= h((string)($row['ProgramName'] ?? '')) ?></td>
              <td><span class="badge text-bg-<?= ((int)($row['OverlayConfiguredFlag'] ?? 0) === 1) ? 'success' : 'secondary' ?>"><?= ((int)($row['OverlayConfiguredFlag'] ?? 0) === 1) ? 'Configured' : 'Not Configured' ?></span></td>
              <td class="text-end"><div class="d-inline-flex gap-1"><?php if ((int)($row['SubProgramID'] ?? 0) > 0): ?><a href="index.php?route=strategy-setup/sub-program-form&id=<?= (int)($row['SubProgramID'] ?? 0) ?>" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil-square"></i></a><?php else: ?><a href="index.php?route=strategy-setup/sub-program-form&data_object_code=<?= urlencode((string)($row['OrgUnitDataObjectCode'] ?? '')) ?>&sub_program_code=<?= urlencode((string)($row['SubProgramCode'] ?? '')) ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-sliders"></i></a><?php endif; ?><?php if ((int)($row['SubProgramID'] ?? 0) > 0 && (int)($row['ActiveFlag'] ?? 0) === 1): ?><form method="post" action="index.php?route=strategy-setup/delete-sub-program" onsubmit="return confirm('Archive this subprogram record?');"><input type="hidden" name="_csrf" value="<?= $csrf ?>"><input type="hidden" name="id" value="<?= (int)($row['SubProgramID'] ?? 0) ?>"><button type="submit" class="btn btn-sm btn-outline-danger"><i class="bi bi-archive"></i></button></form><?php endif; ?></div></td>
            </tr>

...
IF OBJECT_ID('tempdb..#ToAssign') IS NOT NULL DROP TABLE #ToAssign;
...
INTO #ToAssign
FROM Recommended r
INNER JOIN dbo.tblRoles rr
    ON rr.RoleName = r.FunctionalRoleName
LEFT JOIN ExistingFunctional ef
    ON ef.UserID = r.UserID
   AND ef.FunctionalRoleName = r.FunctionalRoleName
WHERE ef.FunctionalRoleName IS NULL;

SELECT
    UserID,
    UserLabel,
    FunctionalRoleName AS RoleToAssign
FROM #ToAssign
ORDER BY UserLabel, FunctionalRoleName;

IF @ExecuteApply = 1
BEGIN
    INSERT INTO dbo.tblUserRoles (UserID, RoleID)
    SELECT ta.UserID, ta.RoleID
    FROM #ToAssign ta
    LEFT JOIN dbo.tblUserRoles existing
        ON existing.UserID = ta.UserID
       AND existing.RoleID = ta.RoleID
    WHERE existing.UserID IS NULL;
END;

SELECT
    CASE WHEN @ExecuteApply = 1 THEN N'APPLIED' ELSE N'PREVIEW_ONLY' END AS RunMode,
    COUNT(*) AS RecommendedAssignments
FROM #ToAssign;
GO
